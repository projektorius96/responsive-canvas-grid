 ...
 * @param {Array} `gridcellMatrix` - one dimensional array (range) generated by exported `setRange` call
 * @returns {Boolean} `true` and at the same time does modify `ctx` passed to the `gridGenerator`   call
 * */
export function gridGenerator({ctx, gridcellDim, gridcellMatrix}) {
    ...
    ctx.setTransform(...[1, 0, 0, 1, 0, 0].map((abcdef)=> abcdef = abcdef * window.devicePixelRatio ));
